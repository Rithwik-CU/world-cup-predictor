...
# ==========================================
# 3. UPGRADE 3: PERSISTENT STORAGE (SQLite)
# ==========================================
def init_db():
...
# ==========================================
# 4. DEFENSIVE AI ENGINE (WITH FORM & EVALUATION)
# ==========================================
class EloSystem:
    ...
    def get_rating(self, team):
        ...
        base_elo = self.ratings.get(team, 1500)
        return base_elo + self.get_form_bonus(team)
    ...
    def get_form_bonus(self, team):
        ...
        return (win_rate - 0.5) * 80  # Max +40 for winning streak, -40 for losing streak
    ...
    def update_ratings(self, team_a, team_b, goal_a, goal_b, tournament, is_neutral=False):
        ...
        ra_raw = self.ratings.get(team_a, 1500)
        rb_raw = self.ratings.get(team_b, 1500)
        
        # --- Model Evaluation Tracking ---
        expected_a = 1 / (1 + 10 ** ((rb_raw - ra_raw) / 400))
        actual_a = 1 if goal_a > goal_b else (0 if goal_a < goal_b else 0.5)
        if (expected_a >= 0.5 and actual_a > 0.5) or (expected_a < 0.5 and actual_a < 0.5) or (actual_a == 0.5):
        ...
        self.ratings[team_a] = ra_raw + k * g_mult * (wa - ea)
        self.ratings[team_b] = rb_raw + k * g_mult * (wb - eb)
        
        # --- Form Tracking Update ---
        ...
        self.recent_form[team_a] = self.recent_form[team_a][-5:]
...
def load_and_train_elo():
    ...
    if os.path.exists(csv_filename):
        try:
            df_main = pd.read_csv(csv_filename, low_memory=False).dropna(subset=['home_score', 'away_score', 'home_team', 'away_team'])
        except Exception:
            pass
            
    # Load Persisted Matches from SQLite
    df_db = pd.read_sql_query("SELECT * FROM matches", conn)
    df_db['tournament'] = 'User Logged'
    df_db['neutral'] = True
    
    if df_main is not None:
        df_combined = pd.concat([df_main, df_db], ignore_index=True)
        for row in df_combined.itertuples(index=False):
            elo.update_ratings(row.home_team, row.away_team, int(row.home_score), int(row.away_score), getattr(row, 'tournament', 'Friendly'), is_neutral=getattr(row, 'neutral', True))
        return elo, f"✅ Trained on {len(df_main):,} historical & {len(df_db)} database matches!"
    elif len(df_db) > 0:
        for row in df_db.itertuples(index=False):
            elo.update_ratings(row.home_team, row.away_team, int(row.home_score), int(row.away_score), 'User Logged', True)
        return elo, f"⚠️ 'results.csv' missing. Trained on {len(df_db)} database matches!"
    else:
...
with tab4:
    ...
    st.write("Evaluates the Elo algorithm's historical prediction accuracy using Backtesting across all processed matches.")
    
    acc = (elo_model.correct_predictions / max(elo_model.total_predictions, 1)) * 100
    
    c1, c2 = st.columns(2)
    c1.metric("Historical Prediction Accuracy", f"{acc:.1f}%")
    c2.metric("Total Matches Backtested", f"{elo_model.total_predictions:,}")
    
    st.divider()
    st.markdown("#### Global Power Rankings (With Recent Form Bonus)")
    rankings = pd.DataFrame([
        {
            "Team": get_flag(t), 
            "Total Elo": int(field_elos[t]), 
            "Base Elo": int(elo_model.ratings.get(t, 1500)), 
            "Form Bonus": int(elo_model.get_form_bonus(t))
        }
        for t in all_teams
    ]).sort_values(by="Total Elo", ascending=False).reset_index(drop=True)
    rankings.index += 1
    st.dataframe(rankings, use_container_width=True)

# ----------------- TAB 5: PERSISTENT DATABASE -----------------
with tab5:
    st.subheader("Log New Match Results")
    st.write("Add real-world match results here. They are permanently saved to a local SQLite database (`user_matches.db`) and immediately retrain the AI!")
    
    with st.form("add_match_form"):
        colA, colB = st.columns(2)
        team_home = colA.selectbox("Home Team", sorted(all_teams), key='h_team', format_func=get_flag)
        score_home = colA.number_input("Home Score", min_value=0, max_value=20, value=0)
        
        team_away = colB.selectbox("Away Team", sorted(all_teams), key='a_team', format_func=get_flag)
        score_away = colB.number_input("Away Score", min_value=0, max_value=20, value=0)
        
        submitted = st.form_submit_button("💾 Save Match & Retrain AI", type="primary")
        if submitted:
            if team_home == team_away:
                ...
                c.execute("INSERT INTO matches VALUES (?, ?, ?, ?, ?)", 
                          ("2026-07-07", team_home, team_away, score_home, score_away))
                conn.commit()
                st.cache_resource.clear() # Clears cache so AI retrains instantly
                st.success(f"✅ Logged: {team_home} {score_home} - {score_away} {team_away}. AI Retrained!")
                st.rerun()
